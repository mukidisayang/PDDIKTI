...
**/
	...
	function isValidEntitasAdd($ret, $record) {
		...
		$nm_kls = escapeLower( $record['nm_kls'] );
		$id_mk = escapeLower( $record['id_mk'] );
		$id_sms = escapeLower( $record['id_sms'] );
		$id_smt = escapeLower( $record['id_smt'] );

		if (( ( ( !$nm_kls || !$id_mk ) || !$id_sms ) || !$id_smt )) {
			setErrorStatus2( $ret, '701' );
			return false;
		}

		$sql = 'select 1 from ' . $g_tab . ' 
		where lower(id_mk)=\'' . $id_mk . '\' 
			and id_sms=\'' . $id_sms . '\' 
			and lower(id_smt)=\'' . $id_smt . '\' 
			and soft_delete=0';

		if (dbGetOne( $sql )) {
			setErrorStatus2( $ret, '700' );
			return false;
		}
	...
	function isValidEntitasUpdate($ret, $where) {
		...
		if (!$num) {
			setErrorStatus2( $ret, '702' );
			return false;
		}


		if (1 < $num) {
			setErrorStatus2( $ret, '703' );
			return false;
		}
