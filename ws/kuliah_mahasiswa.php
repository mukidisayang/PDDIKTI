...
**/
	...
	function thisPK() {
		return array( 'id_smt', 'id_reg_pd' );
	}

	function globalKeys() {
		$g_keys = array( 'id_smt', 'id_reg_pd' );
		return $g_keys;
	}
	...
	function isValidEntitasAdd($ret, $record) {
		...
		$id_reg_pd = escapeLower( $record['id_reg_pd'] );
		$id_stat_mhs = escapeLower( $record['id_stat_mhs'] );

		if (( ( !$id_smt || !$id_reg_pd ) || !$id_stat_mhs )) {
		...
		where lower(id_smt)=\'' . $id_smt . '\' 
			and id_reg_pd=\'' . $id_reg_pd . '\' 
			and soft_delete=0';

		if (dbGetOne( $sql )) {
			setErrorStatus2( $ret, '730' );
			return false;
		}
